...
Use `condor_watch_q` to watch the jobs until they finish. In a separate window, use `tail --lines=500 -f goatbrot.dag.dagman.out` to watch what DAGMan does.
...
Look at the rescue DAG file. It's called a rescue DAG because it indicates what part of the DAG has already been completed.
...
In this exercise, we deliberately caused the "montage" node of the DAG to fail, fixed the issue, and then reran the DAG starting from the rescue DAG.
What if one of the earlier "goatbrot" nodes failed instead? Would the "montage" node run, or not? Will there be a rescue DAG, and if so, what would it look like?
Once you've made some hypotheses, repeat the exercise but introduce a failure into the arguments of one of the goatbrot jobs. 
Do your predictions hold true?
