...
>
> Using p098-words.txt, a 16K text file containing nearly two-thousand common English words, find all the square
...
The first step in our workflow takes an input file with a list of words (`p098-words.txt`)
and extracts all of the anagrams using the `find-anagrams.py` script.
...
The next step in the workflow uses the `max-square.py` script to find the largest square number,
if any, for a given anagram word pair.
Let's submit jobs that run `max-square.py` for all of the anagram word pairs (i.e., one job per word pair)
