...
For example, suppose we want to list all slots that are running CentOS9 (operating system) and have at least 16 GB memory available. Note that memory is reported in units of Megabytes. The command is:

``` console
[username@ap40]$ condor_status -constraint 'OpSysAndVer == "CentOS9" && Memory >= 16000'
```
...
If you are interested in learning more about writing ClassAd expressions, look at the HTCondor Manual (linked in the [References](#references) section). This is advanced material, so it is not required. But if you do, take some time to practice writing expressions for the `condor_status -constraint` command.
...
-   Read about [ClassAd expressions](https://htcondor.readthedocs.io/en/latest/classads/classad-mechanism.html) in the HTCondor Manual
