...
| `tmp/`, `var_tmp/`| Directories in which to put temporary files |
...
Next, you will run a job that requires an input file. 
...
1.  Make the file executable with the `chmod` command.
1.  Download the input file for the script (263K lines, ~1.4 MB) and save it in your submit directory:

    ```
    [username@ap40]$ wget http://osg-htc.org/school-2026/materials/htcondor/files/intro-2.1-words.txt
    ```

1. Test the expected behavior of `freq.py`.
    
    ```
    ./freq.py intro-2.1-words.txt
    ```

1.  Create a submit file for the `freq.py` executable.
1.  Modify the `transfer_input_files` line to tell HTCondor to transfer the input files to the job:

        :::file
        transfer_input_files = freq.py, intro-2.1-words.txt
...
1.  Create a submit file and executable script that transfers the input file and copies it to another name (as if doing `cp input.txt output.txt` on the command line)
...
1.  Write a submit file that transfers any input file and runs `output.sh` on it (remember to pass the input filename as an argument).
